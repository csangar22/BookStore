...
$(document).ready(function() {
    // Al hacer clic en el botón con la clase .btn-review, muestra el modal de reseñas
    $(".btn-review").click(function() {
        $("#review-modal").modal("show");
    });
    // Al enviar el formulario de reseña
    $("#review-form").submit(function(event) {
        ...
        $.ajax({
            type: "POST",
            url: "php/submit_review.php",// URL del script en el servidor que procesará la reseña
            data: formData,// Datos del formulario serializados
            success: function(response) {
                $("#review-modal").modal("hide");
                $("<div class='alert alert-success'>" + response + "</div>").insertAfter(".book-details").fadeOut(3000);// Muestra y desvanece el mensaje
            },
            error: function() {
                // Si ocurre un error, muestra un mensaje de error
                $("<div class='alert alert-danger'>Error al enviar la reseña</div>").insertAfter(".book-details").fadeOut(3000);// Muestra y desvanece el mensaje
            }
        });
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    // Obtiene el botón de agregar al carrito y le añade un evento de clic
    var addToCartBtn = document.getElementById('add-to-cart');
    addToCartBtn.addEventListener('click', function() {
        var isbn = document.getElementById('isbn').value;// Obtiene el valor del ISBN del libro
        addToCart(isbn);// Llama a la función para agregar el libro al carrito
    });
    // Función para agregar un libro al carrito
    function addToCart(isbn) {
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'php/add_to_cart.php', true);// Configura la solicitud POST
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');// Configura el tipo de contenido

        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                // Si la solicitud es exitosa, muestra una alerta y redirige al usuario
                var response = "Libro agregado a la cesta";
                alert(response);
                window.location.href = 'libros.php';// Redirige a la página de libros
            }
        };
        xhr.send('isbn=' + isbn);// Envía la solicitud con el ISBN del libro
    }
});
</script>
</body>
